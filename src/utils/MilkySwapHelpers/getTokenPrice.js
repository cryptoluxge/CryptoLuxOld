...
import { getFactoryContract, getLpContract, getERC20Contract } from "utils/MilkySwapHelpers/contractHelpers";
...
export const getTokenPriceInUSD = async (contractAddress) => {
  const web3 = new Web3("https://rpc-mainnet-cardano-evm.c1.milkomeda.com");
  const getUsdPair = await factoryContract.methods.getPair(mainnetTokens.multiUSDT.contractAddress, contractAddress).call();
...
export const getTokenPriceForFarm = async (contractAddress, decimals) => {
  const web3 = new Web3("https://rpc-mainnet-cardano-evm.c1.milkomeda.com ");
  if (contractAddress === mainnetTokens.multiUSDC.contractAddress) {
    const usdcprice = await getUsdcPrice();
    return usdcprice;
  }

  if (contractAddress === mainnetTokens.multiUSDT.contractAddress) {
  ...
  if (contractAddress === mainnetTokens.MILKY.contractAddress) {
    const milkyPrice = await useMilkyPrice();
    return milkyPrice;
  }
