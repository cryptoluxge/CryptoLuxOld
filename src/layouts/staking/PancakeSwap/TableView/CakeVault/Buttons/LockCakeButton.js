...
function LockCakeButton() {
  ...
  async function stake() {
    const MIN_LOCK_DURATION = 604800; // 1 კვირა
    ...
    if (stakeValue === " ") {
      console.log("შეიყვანეთ რაოდენობა! 1");
    } else if (Number(stakeValue) <= 0) {
      console.log("შეიყვანეთ რაოდენობა! 2");
      ...
      <Modal open={openWallet} onClose={handleCloseWallet} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <SuiBox sx={style}>
          ...
          <SuiBox mt={2}>
            <Grid container spacing={2}>
              <Grid item>
                <SuiTypography sx={{ fontSize: 14 }}>დალუქვის ხანგრძლივობა:</SuiTypography>
              </Grid>
            </Grid>
            <Grid container spacing={0}>
              <Grid item xs={12} md={12} lg={12}>
                <SuiBox sx={{ display: "flex", justifyContent: "space-between", cursor: "pointer" }} mt={1}>
                  <SuiBadge onClick={() => setLockedDuration(1)} badgeContent="1კვ" color="primary" variant="contained" container size="md" />
                  <SuiBadge onClick={() => setLockedDuration(5)} badgeContent="5კვ" color="primary" variant="contained" container size="md" />
                  <SuiBadge onClick={() => setLockedDuration(10)} badgeContent="10კვ" color="primary" variant="contained" container size="md" />
                  <SuiBadge onClick={() => setLockedDuration(25)} badgeContent="25კვ" color="primary" variant="contained" container size="md" />
                  <SuiBadge onClick={() => setLockedDuration(52)} badgeContent="52კვ" color="primary" variant="contained" container size="md" />
                </SuiBox>
              </Grid>
            </Grid>
            <Grid container spacing={1} mt={1}>
              <Grid item>
                <SuiInput id="lockDuration" type="number" pattern="[0-9]" dir="rtl" step="1" min="1" onChange={(e) => stakeButtonStateWeeks(e)} placeholder="ხანგრძლივობა" />
              </Grid>
              <Grid item>
