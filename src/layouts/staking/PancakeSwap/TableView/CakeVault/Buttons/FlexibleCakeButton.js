...
function FlexibleCakeButton() {
  ...
  async function setMaxCakes() {
    document.getElementById("flexibleCakeToDeposit").value = cakeBalance;
  }

  async function stakeButtonState(e) {
    const value = Number(e.target.value);
    if (value > Number(cakeBalance)) {
      document.getElementById("CakeVaultFlexibleStakeButton").disabled = true;
    } else if (value <= 0) {
      document.getElementById("CakeVaultFlexibleStakeButton").disabled = true;
    } else if (Number(cakeBalance) > value > 0) {
      document.getElementById("CakeVaultFlexibleStakeButton").disabled = false;
    }
  }

  async function stake() {
    const stakeValue = document.getElementById("flexibleCakeToDeposit").value;
    if (stakeValue === " ") {
        ...
        <SuiBox sx={style}>
          ...
          <SuiBox>
            ...
            <Grid container spacing={1}>
              <Grid item>
                <SuiInput id="flexibleCakeToDeposit" onChange={(e) => stakeButtonState(e)} placeholder="CAKE რაოდენობა" />
              </Grid>
              <Grid item>
          ...
          <SuiBox>
            <SuiBox mb={1}>
              <SuiButton id="CakeVaultFlexibleStakeButton" fullWidth onClick={() => stake()} variant="gradient" color="primary">
                STAKE
              </SuiButton>
            </SuiBox>
...
export default FlexibleCakeButton;
