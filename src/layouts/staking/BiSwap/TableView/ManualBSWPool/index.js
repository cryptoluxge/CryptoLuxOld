...
import { getUserData, getManualBSWTVL } from "./Helpers";
...
function CakeVault() {
  ...
  async function getTvl() {
    const tvl = await getManualBSWTVL();
  ...
  async function userData() {
    ...
    console.log("ManualPool: ", data);
    setPendingBsw(Number(data.pendingBSW));
    setStakedBsw(Number(data.currentBsw));
  }
    ...
    <React.Fragment key="key">
      <TableRow sx={{ "& > *": { borderBottom: "unset" }, cursor: "pointer" }} onClick={() => setOpen(!open)}>
        ...
        <TableCell align="left">
          ...
          {Number(pendingBsw) >= 0 && active === true ? (
