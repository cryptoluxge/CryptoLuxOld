...
function HarvestButton({ pid }) {
  ...
  const MasterMilkyContract = getMasterMilkyContract(chainId);

  async function checkPendingRewards() {
    const balance = await MasterMilkyContract.methods.pendingMilky(pid, account).call();
    if (balance === "0") {
  ...
  async function harvestRewards() {
    await MasterMilkyContract.methods
      .deposit(pid, 0)
      .send({ from: account })
