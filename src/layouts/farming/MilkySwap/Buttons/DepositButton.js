...
import { getLpContract, getMasterMilkyContract } from "utils/MilkySwapHelpers/contractHelpers";
...
function DepositButton({ pid, lpAddress, name, token, quoteToken }) {
  ...
  async function getMilkyBalance() {
    const balance = await FarmContract.methods.balanceOf(account).call();
    setMilkyBalance(web3.utils.fromWei(balance, "ether"));
  }

  async function setMaxMilkys() {
    document.getElementById("milkyToDeposit").value = milkyBalance;
  }

  async function stakeButtonState(e) {
    const value = Number(e.target.value);
    if (value > Number(milkyBalance)) {
      document.getElementById(`stakeButton${pid}`).disabled = true;
    } else if (value <= 0) {
      document.getElementById(`stakeButton${pid}`).disabled = true;
    } else if (Number(milkyBalance) > value > 0) {
      document.getElementById(`stakeButton${pid}`).disabled = false;
    }
  }

  async function stake() {
    const stakeValue = document.getElementById("milkyToDeposit").value;
    if (stakeValue === " ") {
        ...
        <SuiBox sx={style}>
          ...
          <SuiBox>
            ...
            <Grid container spacing={1}>
              <Grid item>
                <SuiInput id="milkyToDeposit" onChange={(e) => stakeButtonState(e)} placeholder={`${name} რაოდენობა`} />
              </Grid>
              <Grid item>
          ...
          <SuiBox>
            <SuiTypography sx={{ fontSize: 14 }} mt={1} variant="h6" component="h2">
              ბალანსი: {milkyBalance}
            </SuiTypography>
          </SuiBox>
          <Divider />
          <SuiBox sx={{ display: "flex", justifyContent: "center" }}>
            <SuiButton href={`https://milkyswap.exchange/add/${token}/${quoteToken}`} target="_blank" variant="outlined" color="primary">
              Get {name}
            </SuiButton>
