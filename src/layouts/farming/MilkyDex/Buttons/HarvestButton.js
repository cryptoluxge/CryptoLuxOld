...
function HarvestButton({ pid }) {
  ...
  async function checkPendingRewards() {
    const balance = await MilkyChefContract.methods.pendingReward(pid, account).call();
    if (balance === "0") {
  ...
  async function harvestRewards() {
    /* const pendingRewards = await MilkyChefContract.methods.pendingReward(0, account).call(); */
