...
// Contracts
import { useTotalLiquidityUSD, usePoolMultiplier, usePoolWeight, getUserPendingYEL } from "utils/YelFinanceHelpers/farmHelpers";
import { getFarmApr } from "utils/YelFinanceHelpers/apr";
// Hooks
...
function PoolRow({ pid, lpName, lpAddress, tokenAddress, quoteTokenAddress }) {
  ...
  useEffect(async () => {
    if (Number(totalLiquidity) > 0) {
      ...
      const apr = getFarmApr(poolWeight, yelPrice, totalLiquidity);
      setPoolApr(apr);
    }
  }, [totalLiquidity]);
      ...
      <TableRow sx={{ "& > *": { borderBottom: "unset" }, cursor: "pointer" }} onClick={() => setOpen(!open)}>
        ...
        <TableCell align="left">
          ...
          {pendingYel >= 0 && active === true ? (
            <SuiTypography fontWeight="bold" textGradient={Number(pendingYel) > 0 ? true : null} color={Number(pendingYel) > 0 ? "primary" : "dark"} sx={{ fontSize: 14 }}>
              {Number(pendingYel).toFixed(3)} (${(pendingYel * yelPrice).toLocaleString("en-US")})
            </SuiTypography>
          ) : (
            <LoadingAnimation />
          )}
        ...
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={7}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Grid container spacing={2} mt={1} mb={1}>
              <Grid item>
                <DetailsBox pid={pid} lpAddress={lpAddress} poolApr={Number(poolApr)} lpName={lpName} />
              </Grid>
