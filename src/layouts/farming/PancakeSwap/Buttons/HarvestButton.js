...
function HarvestButton({ pid }) {
  ...
  async function checkPendingRewards() {
    const balance = await ManualCAKEContract.methods.pendingCake(pid, account).call();
    if (balance === "0") {
  ...
  async function harvestRewards() {
    /* const pendingRewards = await ManualCAKEContract.methods.pendingCake(0, account).call(); */
